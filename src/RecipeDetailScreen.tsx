    ...
    <div className="min-h-screen bg-white max-w-md mx-auto pb-10">
      <div className="relative h-[300px] bg-gray-100">
        ...
        <button onClick={onBack} aria-label="Back" className="w-10 h-10 rounded-full bg-white/90 flex items-center justify-center" style={{ position: 'absolute', top: 20, left: 16, zIndex: 20 }}>
          <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="#1F1F1F" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
        </button>
        <button type="button" aria-label="Share recipe" className="w-10 h-10 rounded-full bg-white/90 flex items-center justify-center" style={{ position: 'absolute', top: 20, right: 16, zIndex: 20 }}>
          <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="#1F1F1F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3" /><circle cx="6" cy="12" r="3" /><circle cx="18" cy="19" r="3" /><line x1="8.59" y1="10.51" x2="15.42" y2="6.49" /><line x1="8.59" y1="13.49" x2="15.42" y2="17.51" /></svg>
        </button>
      </div>
      ...
      <div className="px-4 pt-5">
        ...
        {activeTab === 'Ingredients' && (
          <section className="mt-6">
            ...
            <div className="space-y-6">
              {groupedIngredients.map(([group, items]) => <div key={group}>{group !== 'Main' && <h3 className="font-semibold text-[15px] mb-2" style={{ color: '#1F1F1F' }}>{group}</h3>}<div className="divide-y" style={{ borderColor: '#EAEAEA' }}>{items.map((item) => <div key={item.id} className="flex justify-between gap-4 py-3" style={{ borderColor: '#EAEAEA' }}><div className="min-w-0"><p className="text-[15px]" style={{ color: '#1F1F1F' }}>{item.ingredients?.name || 'Ingredient'}{item.optional ? <span className="text-[12px] ml-1" style={{ color: '#6F6F6F' }}>(optional)</span> : null}</p>{item.substitute && <p className="text-[12px] mt-0.5" style={{ color: '#6F6F6F' }}>Sub: {item.substitute}</p>}</div><p className="text-[14px] whitespace-nowrap font-medium" style={{ color: '#1F1F1F' }}>{formatAmount(item.amount)} {item.unit || ''}</p></div>)}</div></div>)}
            </div>
          </section>
        )}

        {activeTab === 'Steps' && <section className="mt-6"><h2 className="font-semibold text-[20px] mb-4" style={{ color: '#1F1F1F' }}>Steps</h2><div className="space-y-7">{steps.map((step) => <article key={step.id}>{step.step_image && <div className="rounded-[16px] overflow-hidden bg-gray-100 mb-3"><img src={imageUrl(step.step_image)} alt={step.title || `Step ${step.step_number}`} className="w-full aspect-[4/3] object-cover" /></div>}<div className="flex gap-3"><div className="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-white text-[14px] font-semibold" style={{ backgroundColor: '#F26B21' }}>{step.step_number}</div><div className="pt-0.5"><div className="flex items-center gap-2 flex-wrap"><h3 className="font-semibold text-[17px]" style={{ color: '#1F1F1F' }}>{step.title || `Step ${step.step_number}`}</h3>{step.step_time_minutes ? <span className="text-[12px] px-2 py-0.5 rounded-full" style={{ backgroundColor: '#FFF0E6', color: '#F26B21' }}>{formatTime(step.step_time_minutes)}</span> : null}</div><p className="text-[15px] leading-6 mt-1.5" style={{ color: '#6F6F6F' }}>{step.instruction}</p></div></div></article>)}</div></section>}
        ...
        {recommendations.length > 0 && (
          <section className="pt-6 border-t" style={{ borderColor: '#EEEEEE', marginTop: 50 }}>
            ...
            <button type="button" onClick={onBrowse} className="w-full h-12 mt-8 rounded-[14px] border text-[14px] font-semibold" style={{ borderColor: '#F26B21', color: '#F26B21', backgroundColor: '#FFFFFF' }}>See more recipes</button>
          </section>
        )}
      </div>
    </div>
